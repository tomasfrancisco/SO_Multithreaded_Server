...
void end_receiver_process(int sig);
...
void receiver_process(int port, int size) {
  struct sockaddr_in client_name;
  socklen_t client_name_len = sizeof(client_name);
  requests_buffer = buffer_create(size);

  signal(SIGINT, end_receiver_process);
  ...
  while (1) {
    ...
    /*printf("Add new connection to buffer\n");
    buffer_add_node(requests_buffer, new_conn, STATIC, "index.html");*/
...
void execute_script(int socket)
{
  ...
  sprintf(buf_tmp,"../data/scripts/%s",req_buf);
  ...
  if(((fp=fopen(buf_tmp,"rt"))==NULL)) {
    // Script not found, send error to client
...
void send_page(int socket)
{
  ...
  // Searchs for page in directory htdocs
  sprintf(buf_tmp,"../data/web_pages/%s",req_buf);
